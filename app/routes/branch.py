...
async def list_branches(session: AsyncSession = Depends(get_session),current_user: User = Depends(require_admin_or_senior_editor)):
    return await get_all_branches(session)

@router.get("/{branch_id}", response_model=BranchRead, name="Get Branch")
async def get_branch(branch_id: UUID, session: AsyncSession = Depends(get_session),current_user: User = Depends(require_admin_or_senior_editor)):
...
async def update(branch_id: UUID, update_data: BranchUpdate, session: AsyncSession = Depends(get_session),current_user: User = Depends(require_admin_or_senior_editor)):
    branch = await get_branch_by_id(session, branch_id)
    if not branch:
        raise HTTPException(status_code=404, detail="Branch not found")
    return await update_branch(session, branch, update_data)

@router.delete("/{branch_id}", status_code=204, name="Delete Branch")
async def delete(branch_id: UUID, session: AsyncSession = Depends(get_session),current_user: User = Depends(require_admin_or_senior_editor)):
    branch = await get_branch_by_id(session, branch_id)
    if not branch:
        raise HTTPException(status_code=404, detail="Branch not found")
